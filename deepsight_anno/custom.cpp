...
class roiMode : public taskMode {
 ...
 private:
  ...
  bool modifyROI(const QJsonArray& aModification, QJsonObject& aROI,
                 QString& aPath);
...
QJsonObject roiMode::getImages() { return m_task->value("images").toObject(); }

int roiMode::getROICount(const QJsonObject& aROI, const QString& aTag) {
  auto shps = getShapes(aROI);
  auto tg = "roi_" + aTag + "_";
  int cnt = 0;
  while (shps.contains(tg + QString::number(cnt))) cnt++;
  return cnt;
}
...
QJsonObject roiMode::parseParam(const QJsonObject& aROI) {
  ...
  for (auto i : shps.keys()) {
    ...
    pts = pts[0].toArray();
    std::vector<double> val = {pts[0].toDouble(), pts[1].toDouble(),
                               pts[4].toDouble(), pts[5].toDouble()};
    rea::tryFind(&rois, i.split("_")[1])
        ->push_back(rea::JArray(val[0] / w, val[1] / h, (val[2] - val[0]) / w,
                                (val[3] - val[1]) / h));
  }
  QJsonObject ret;
  for (auto i : rois.keys()) {
    QJsonArray ret0;
    for (auto j : rois.value(i)) ret0.push_back(j);
    ret.insert(i, ret0);
  }
  return ret;
}
...
void roiMode::initialize() {
          ...
          if (getTaskID() != "") {
            ...
            if (isCurrentMode("roi"))
              aInput->out<QJsonObject>(
                  rea::Json("visible", true, "count",
                            getROICount(roi, m_local ? getImageID() : "whole")),
                  "updateROIGUI");
            aInput->out<rea::stgJson>(rea::stgJson(*m_task, pth),
                                      s3_bucket_name_() + "writeJson");
          }
        } else if (pth != cur && belongThisMode("roi", pth)) {
          aInput->cache<QJsonArray>(aInput->data())
              ->out<rea::stgJson>(rea::stgJson(QJsonObject(), pth));
        }
      },
      rea::Json("name", "roi_tryModifyCurrentModel"));

  // interface remote save
  rea::pipeline::add<rea::stgJson>(
      [this](rea::stream<rea::stgJson>* aInput) {
        auto dt = aInput->data().getData();
        auto pth = QString(aInput->data());
        auto roi = getROI(dt);
        modifyROI(aInput->cacheData<QJsonArray>(0), roi, pth);
        setROI(dt, roi);
        aInput->out<rea::stgJson>(rea::stgJson(dt, pth),
                                  s3_bucket_name_() + "writeJson");
      },
  ...
  rea::pipeline::find("startJob")
      ->next(rea::pipeline::add<QJsonObject>(
          [this](rea::stream<QJsonObject>* aInput) {
  ...
  rea::pipeline::add<double>(
      [this](rea::stream<double>* aInput) {
        ...
        for (int i = cur; i < cnt; ++i) {
          ...
          auto ad = [shp, id, val]() {
            rea::pipeline::run(
                "updateQSGAttr_taskimage_gridder0",
                rea::Json("key", rea::JArray("objects"), "type", "add", "tar",
                          shp, "val", val, "cmd", true, "id", id));
          };
          auto rm = [shp, id]() {
            rea::pipeline::run(
                "updateQSGAttr_taskimage_gridder0",
                rea::Json("key", rea::JArray("objects"), "type", "del", "tar",
                          shp, "cmd", true, "id", id));
          };
          ad();
  ...
  rea::pipeline::add<bool>(
      [this](rea::stream<bool>* aInput) {
        m_local = aInput->data();
        auto roi = getROI(*m_task);
        auto shps = getShapes(roi);
  ...
  // auto roi
  rea::pipeline::add<QJsonObject>(
      [this](rea::stream<QJsonObject>* aInput) {
        auto imgs = getImages();
        QJsonArray lst;
        for (auto i : imgs.keys()) lst.push_back(i);
        aInput
            ->setData(rea::Json("roi", getShapes(getROI(*m_task)), "images",
                                lst, "root",
                                "project/" + getProjectID() + "/image/"))
            ->out();
      },
...
std::function<void(void)> roiMode::prepareQSGObjects(QJsonObject& aObjects) {
  ...
  auto roi_cfg =
      rea::Json("count", aObjects.size() - 1, "visible", true, "l", m_local);
  return
      [roi_cfg]() { rea::pipeline::run<QJsonObject>("updateROIGUI", roi_cfg); };
}
...
QJsonObject roiMode::getDefaultROI() {
  ...
  return rea::Json(
      "type", "poly", "color", "pink", "points",
      rea::JArray(QJsonArray(), rea::JArray(0, 0, w, 0, w, h, 0, h, 0, 0)));
}

bool roiMode::modifyROI(const QJsonArray& aModification, QJsonObject& aROI,
                        QString& aPath) {
  bool modified = false;
...
static rea::regPip<QQmlApplicationEngine*> init_roi_mode(
