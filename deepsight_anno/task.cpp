...
private:
    ...
    QString m_task_id = "";
    QVector<QString> m_project_label_groups;
    ...
    void taskManagement(){
        ...
        rea::pipeline::add<QJsonObject>([this](rea::stream<QJsonObject>* aInput){
            ...
            if (dt.value("id") != m_task_id){
                ...
                auto lbls = dt.value("labels").toObject();
                ...
                m_project_label_groups.clear();
                for (auto i : lbls.keys())
                    m_project_label_groups.push_back(i);
    ...
    void labelManagement(){
        ...
        rea::pipeline::find("project_label_updateListView")
            ->next(rea::pipeline::add<QJsonObject>([this](rea::stream<QJsonObject>* aInput){
                if (m_task_id == "")
                    return;
                auto lbls = aInput->data().value("data").toArray();
                m_project_label_groups.clear();
                for (auto i : lbls)
                    m_project_label_groups.push_back(i.toObject().value("entry").toArray()[0].toString());
                aInput->out<QJsonObject>(prepareLabelListGUI(getLabels()), "task_label_updateListView");
            }))
            ->next("task_label_updateListView");

        //update project label
        rea::pipeline::find("updateProjectLabelGUI")
            ->next(rea::pipeline::add<QJsonObject>([this](rea::stream<QJsonObject>* aInput){
                if (m_task_id == "")
                    return;
            }))
        ...
        rea::pipeline::add<QJsonObject>([](rea::stream<QJsonObject>* aInput){
            ...
            ->next(rea::pipeline::add<std::vector<QJsonArray>>([this](rea::stream<std::vector<QJsonArray>>* aInput){
                ...
                auto grps = getLabels();
                auto grp = m_project_label_groups.at(sel);
