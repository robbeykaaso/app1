...
cv::Mat fsStorage::readCVMat(const QString& aPath) {
  auto ret = cv::imread((stgRoot(aPath)).toLocal8Bit().toStdString().data(),
                        cv::IMREAD_UNCHANGED);
...
static rea::regPip<QJsonObject, rea::pipePartial> json2stg(
    [](rea::stream<QJsonObject>* aInput) {
      auto dt = aInput->data();
      aInput->out<rea::stgJson>(rea::stgJson(dt.value("data").toObject(),
                                             dt.value("path").toString()),
                                dt.value("next").toString(),
                                dt.value("param").toObject());
    },
...
/*bool safetyWrite(const QString& aPath, const QByteArray& aData){
    ...
    if (fl.open(QFile::WriteOnly)){
        fl.write(aData);
        fl.close();
        if (!MoveFileExA(tmp.toLocal8Bit().toStdString().data(),
aPath.toLocal8Bit().toStdString().data(), MOVEFILE_REPLACE_EXISTING)){ std::cout
<< "write file error: " << GetLastError() << std::endl; return false;
        }
        return true;
    }
    return false;
